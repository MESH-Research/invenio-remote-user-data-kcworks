...
# more details.
#
# Originally lived in the parent KCWorks repository at
# ``tests/api/test_user_data_sync.py``; moved here so the integration tests
# travel with the package whose behaviour they exercise. Tests covering the
# now-removed ``kcworks.services.accounts.idms.knowledgeCommons_account_setup``
# helper were dropped at move time (the helper was dead code, replaced by
# ``CILogonHelpers.link_user_to_oauth_identifier`` in this package).
...
tests at the top of the module hit the live KC IDP and require
``COMMONS_PROFILES_API_TOKEN`` and network access.
"""
