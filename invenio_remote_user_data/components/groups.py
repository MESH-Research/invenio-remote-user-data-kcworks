...
class GroupsComponent(ServiceComponent):
    ...
    def find_or_create_group(
        ...
        doesn't exist."""
        my_group_role = security_datastore.find_or_create_role(
    ...
    def create_new_group(self, group_name: str, **kwargs) -> Optional[Role]:
        ...
        my_group_role = security_datastore.create_role(
            name=group_name, **kwargs
        )
        # FIXME: Is this right?
        security_datastore.commit()
        if my_group_role is not None:
            if self.logger.debug:
                print(f'Role "{group_name}" created successfully.')
        else:
    ...
    def remove_user_from_group(
        ...
        removed_user = security_datastore.remove_role_from_user(
            user, group_name
        )
        # FIXME: Is this right?
        security_datastore.commit()
